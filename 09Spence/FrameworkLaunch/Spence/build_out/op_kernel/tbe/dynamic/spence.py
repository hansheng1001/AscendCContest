...
@tbe_register.register_operator("Spence", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def spence(x_in__, y_out_, kernel_name="spence", impl_mode=""):
    ...
    origin_func_name = "spence"
    ascendc_src_dir_ex = "spence"
    ascendc_src_dir = "spence"
    ascendc_src_file = "spence.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Spence. kernel name is " + kernel_name)
    op_type = "Spence"
...
def op_select_format(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("op_select_format", "Spence", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "Spence", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
