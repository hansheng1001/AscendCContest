...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnSpenceGetWorkspaceSize(inputTensor_[0], outputTensor_[0],
    ...
    if (aclnnSpence(workspace, workspaceSize, handle, stream) != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
