...
@tbe_register.register_operator("FastGelu", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def fast_gelu(x_in__, y_out_, kernel_name="fast_gelu", impl_mode=""):
    ...
    origin_func_name = "fast_gelu"
    ascendc_src_dir_ex = "fast_gelu"
    ascendc_src_dir = "fast_gelu"
    ascendc_src_file = "fast_gelu.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator FastGelu. kernel name is " + kernel_name)
    op_type = "FastGelu"
...
def op_select_format(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("op_select_format", "FastGelu", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "FastGelu", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
