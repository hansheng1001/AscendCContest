...
public:
  ...
  __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR out) {
    ...
    pipe.InitBuffer(inQueueIN, BUFFER_NUM, this->tileLength * 2 * sizeof(half));
...
private:
  ...
  __aicore__ inline void Compute(int32_t progress) {
    ...
    Sub<half>(outLocal, xLocal, yLocal, this->tileLength);
    Mul<half>(outLocal, outLocal, outLocal, this->tileLength);

    // 需要考虑最后tiling的长度
    if ((1 == this->reduction) || (2 == this->reduction)) {
      ReduceSum<half>(outLocal, outLocal, xLocal, this->tileLength);
    }
...
void mse_loss_do(uint32_t blockDim, void *l2ctrl, void *stream, uint8_t *x,
  ...
  for (int i = 0; i < USE_CORE_NUM; i++) {
    ...
    for (int j = 0; j < sumCount; j++) {
      DataCopy(&t, out + offset + j * sizeof(half), sizeof(half));
      sum += t;
    }

    offset += BLOCK_LENGTH * sizeof(half);
  }

  DataCopy(out, &sum, sizeof(half));
}
#endif