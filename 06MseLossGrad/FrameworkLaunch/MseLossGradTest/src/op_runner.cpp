...
bool OpRunner::RunOp()
{
    ...
    string reduction;
	auto ret = aclnnMseLossGradGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2],opDesc_->reduction, outputTensor_[0],
    ...
    if (aclnnMseLossGrad(workspace, workspaceSize, handle, stream) != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
