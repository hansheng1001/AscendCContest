...
def _build_args(predict_in__, label_in__, x_in__, z_out_, reduction):
    __inputs__ = []
    for arg in [predict_in__, label_in__, x_in__]:
        if arg != None:
    ...
    if reduction != None:
        ...
        attr["name"] = "reduction"
        attr["dtype"] = "str"
        attr["value"] = reduction
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("MseLossGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def mse_loss_grad(predict_in__, label_in__, x_in__, z_out_, reduction="mean", kernel_name="mse_loss_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(predict_in__, label_in__, x_in__, z_out_, reduction)
    ...
    origin_func_name = "mse_loss_grad"
    ascendc_src_dir_ex = "mse_loss_grad"
    ascendc_src_dir = "mse_loss_grad"
    ascendc_src_file = "mse_loss_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator MseLossGrad. kernel name is " + kernel_name)
    op_type = "MseLossGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[predict_in__, label_in__, x_in__], origin_outputs = [z_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(predict_in__, label_in__, x_in__, z_out_, reduction="mean", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(predict_in__, label_in__, x_in__, z_out_, reduction)
    result = check_op_cap("op_select_format", "MseLossGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(predict_in__, label_in__, x_in__, z_out_, reduction="mean", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(predict_in__, label_in__, x_in__, z_out_, reduction)
    result = check_op_cap("get_op_specific_info", "MseLossGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
