...
SupportInfo list0_1 = {inputDesc0_1, 3, outputDesc0_1, 1};
...
[[maybe_unused]] uint32_t NNOPBASE_MseLossGrad = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnMseLossGradTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "MseLossGrad";
    char inputDesc[] = {1, 1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {0};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(predict);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(label);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, predict, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, label, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, x, 2));
    if (reductionOptional) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttr(*executor, static_cast<void*>(reductionOptional), strlen(reductionOptional) + 1, 0));
    } else {
        static char *reductionOptionalDef = "mean";
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttr(*executor, static_cast<void*>(reductionOptionalDef), strlen(reductionOptionalDef) + 1, 0));
    }
