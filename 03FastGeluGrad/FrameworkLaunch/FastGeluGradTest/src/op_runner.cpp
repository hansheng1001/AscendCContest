...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnFastGeluGradGetWorkspaceSize(inputTensor_[0], inputTensor_[1],  outputTensor_[0],
    ...
    if (aclnnFastGeluGrad(workspace, workspaceSize, handle, stream) != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
