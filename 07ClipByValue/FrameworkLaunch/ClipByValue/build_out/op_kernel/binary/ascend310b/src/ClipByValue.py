...
def _build_args(x_in__, minv_in__, maxv_in__, z_out_):
    __inputs__ = []
    for arg in [x_in__, minv_in__, maxv_in__]:
        if arg != None:
...
@tbe_register.register_operator("ClipByValue", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def clip_by_value(x_in__, minv_in__, maxv_in__, z_out_, kernel_name="clip_by_value", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, minv_in__, maxv_in__, z_out_)
    ...
    origin_func_name = "clip_by_value"
    ascendc_src_dir_ex = "clip_by_value"
    ascendc_src_dir = "clip_by_value"
    ascendc_src_file = "clip_by_value.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator ClipByValue. kernel name is " + kernel_name)
    op_type = "ClipByValue"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, minv_in__, maxv_in__], origin_outputs = [z_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, minv_in__, maxv_in__, z_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, minv_in__, maxv_in__, z_out_)
    result = check_op_cap("op_select_format", "ClipByValue", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, minv_in__, maxv_in__, z_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, minv_in__, maxv_in__, z_out_)
    result = check_op_cap("get_op_specific_info", "ClipByValue", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
