...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnClipByValueGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2], outputTensor_[0],
    ...
    if (aclnnClipByValue(workspace, workspaceSize, handle, stream) != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
