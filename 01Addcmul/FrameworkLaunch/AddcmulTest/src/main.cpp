...
OperatorDesc CreateOpDesc() {
  ...
  opDesc.AddInputTensorDesc(dataType, shape.size(), shape.data(), format);
  opDesc.AddInputTensorDesc(dataType, shape.size(), shape.data(), format);
  opDesc.AddInputTensorDesc(dataType, shape_value.size(), shape_value.data(),
                            format);
...
bool SetInputData(OpRunner &runner) {
  ...
  ReadFile("../input/input_z.bin", fileSize, runner.GetInputBuffer<void>(2),
           runner.GetInputSize(2));
  ReadFile("../input/input_value.bin", fileSize, runner.GetInputBuffer<void>(3),
           runner.GetInputSize(3));
  INFO_LOG("Set input success");
  return true;
}

bool ProcessOutputData(OpRunner &runner) {
  WriteFile("../output/output.bin", runner.GetOutputBuffer<void>(0),
