...
def _build_args(value_in__, input_data_in__, x1_in__, x2_in__, y_out_):
    __inputs__ = []
    for arg in [value_in__, input_data_in__, x1_in__, x2_in__]:
        if arg != None:
...
@tbe_register.register_operator("Addcmul", trans_bool_to_s8=False)
...
def addcmul(value_in__, input_data_in__, x1_in__, x2_in__, y_out_, kernel_name="addcmul", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, input_data_in__, x1_in__, x2_in__, y_out_)
    ...
    origin_func_name = "addcmul"
    ascendc_src_dir_ex = "addcmul"
    ascendc_src_dir = "addcmul"
    ascendc_src_file = "addcmul.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Addcmul. kernel name is " + kernel_name)
    op_type = "Addcmul"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[value_in__, input_data_in__, x1_in__, x2_in__], origin_outputs = [y_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(value_in__, input_data_in__, x1_in__, x2_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, input_data_in__, x1_in__, x2_in__, y_out_)
    result = check_op_cap("op_select_format", "Addcmul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(value_in__, input_data_in__, x1_in__, x2_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, input_data_in__, x1_in__, x2_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "Addcmul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
