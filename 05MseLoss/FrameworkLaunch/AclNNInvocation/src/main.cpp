...
OperatorDesc CreateOpDesc() {
  // define operator
  // std::vector<int64_t> shape { 8, (1320*3)*16 }; //16384 //1365*16
  std::vector<int64_t> shape{1, (1 * 2048)}; // 368;{ 1, (31*64+48) }
  ...
  // opDesc.AddOutputTensorDesc(dataType, 1, shape.data(), format);
  return opDesc;
}

bool SetInputData(OpRunner &runner) {
  size_t fileSize = 0;
  ReadFile("../input/input_x.bin", fileSize, runner.GetInputBuffer<void>(0),
           runner.GetInputSize(0));
  ReadFile("../input/input_y.bin", fileSize, runner.GetInputBuffer<void>(1),
           runner.GetInputSize(1));
  INFO_LOG("Set input success");
  return true;
}

bool ProcessOutputData(OpRunner &runner) {
  WriteFile("../output/output_out.bin", runner.GetOutputBuffer<void>(0),
            runner.GetOutputSize(0));

  // 这是为了验证half精度的sum或者是mean
  // WriteFile("../output/output_out.bin", runner.GetOutputBuffer<void>(0), 4);
