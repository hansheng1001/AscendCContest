...
def _build_args(x_in__, y_in__, out_out_, reduction):
    __inputs__ = []
...
@tbe_register.register_operator("MseLoss", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def mse_loss(x_in__, y_in__, out_out_, reduction="mean", kernel_name="mse_loss", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, out_out_, reduction)
    ...
    origin_func_name = "mse_loss"
    ascendc_src_dir_ex = "mse_loss"
    ascendc_src_dir = "mse_loss"
    ascendc_src_file = "mse_loss.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator MseLoss. kernel name is " + kernel_name)
    op_type = "MseLoss"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, y_in__], origin_outputs = [out_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, y_in__, out_out_, reduction="mean", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, out_out_, reduction)
    result = check_op_cap("op_select_format", "MseLoss", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_in__, out_out_, reduction="mean", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, out_out_, reduction)
    result = check_op_cap("get_op_specific_info", "MseLoss", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
