...
bool OpRunner::RunOp() {
  ...
  string reduction;
  auto ret = aclnnScatterMaxGetWorkspaceSize(
      inputTensor_[0], inputTensor_[1], inputTensor_[2], opDesc_->use_locking,
  ...
  INFO_LOG("Execute GetWorkspaceSize success, workspace size %lu",
           workspaceSize);
  ...
  if (aclnnScatterMax(workspace, workspaceSize, handle, stream) !=
      ACL_SUCCESS) {
    (void)aclrtDestroyStream(stream);
  ...
  for (size_t i = 0; i < numInputs_; ++i) {
    ...
    if (aclrtMemcpy(hostInputs_[i], size, devInputs_[i], size, kind) !=
        ACL_SUCCESS) {
