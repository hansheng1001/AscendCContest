...
@tbe_register.register_operator("ScatterMax", trans_bool_to_s8=False)
...
def scatter_max(x_in__, index_in__, update_in__, x_out_, use_locking=False, kernel_name="scatter_max", impl_mode=""):
    ...
    origin_func_name = "scatter_max"
    ascendc_src_dir_ex = "scatter_max"
    ascendc_src_dir = "scatter_max"
    ascendc_src_file = "scatter_max.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator ScatterMax. kernel name is " + kernel_name)
    op_type = "ScatterMax"
...
def op_select_format(x_in__, index_in__, update_in__, x_out_, use_locking=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, index_in__, update_in__, x_out_, use_locking)
    result = check_op_cap("op_select_format", "ScatterMax", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, index_in__, update_in__, x_out_, use_locking=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, index_in__, update_in__, x_out_, use_locking)
    result = check_op_cap("get_op_specific_info", "ScatterMax", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
