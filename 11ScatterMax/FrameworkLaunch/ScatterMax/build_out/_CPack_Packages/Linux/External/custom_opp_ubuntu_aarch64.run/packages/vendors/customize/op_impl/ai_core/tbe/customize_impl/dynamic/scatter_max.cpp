...
public:
  ...
  Init(GM_ADDR x, GM_ADDR index, GM_ADDR update, GM_ADDR y, uint32_t dataType,
    ...
    pipe.InitBuffer(iBuf, 1, this->indexLength * sizeof(uint32_t));
    pipe.InitBuffer(uBuf, 1, this->indexLength * sizeof(DTYPE_X));
  }
    ...
    if (0 == this->updateType) {
      ...
      for (int32_t i = 0; i < this->indexLength; i++) {
        ...
        if ((progress * this->tileLength <= index) &&
          ...
          x1 = (x1 >= this->attrScaler) ? x1 : this->attrScaler;
          newValue = (DTYPE_X)x1;
...
extern "C" __global__ __aicore__ void scatter_max(GM_ADDR x, GM_ADDR index,
  ...
  op.Init(
      x, index, update, x_ref, tiling_data.dataType_, tiling_data.indexLength_,
      tiling_data.updateType_, tiling_data.blockLength, tiling_data.tileNum,
...
void scatter_max_do(uint32_t blockDim, void *l2ctrl, void *stream, GM_ADDR x,
  ...
  scatter_max<<<blockDim, l2ctrl, stream>>>(x, index, update, y, workspace,
                                            tiling);
