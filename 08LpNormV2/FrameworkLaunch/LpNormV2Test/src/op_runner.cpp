...
bool OpRunner::RunOp()
{
    ...
    if (opDesc_->axesOptional.size() > 0)
    {
        axes = aclCreateIntArray(opDesc_->axesOptional.data(), opDesc_->axesOptional.size());
    }
    
    auto ret = aclnnLpNormV2GetWorkspaceSize(inputTensor_[0], opDesc_->pOptional, axes, opDesc_->keepdimOptional, opDesc_->epsilonOptional, outputTensor_[0],
    ...
    if (aclnnLpNormV2(workspace, workspaceSize, handle, stream) != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
