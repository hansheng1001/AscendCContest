...
def _build_args(x_in__, y_out_, p, axes, keepdim, epsilon):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("LpNormV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def lp_norm_v2(x_in__, y_out_, p=2, axes=[], keepdim=False, epsilon=1e-12, kernel_name="lp_norm_v2", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, p, axes, keepdim, epsilon)
    ...
    origin_func_name = "lp_norm_v2"
    ascendc_src_dir_ex = "lp_norm_v2"
    ascendc_src_dir = "lp_norm_v2"
    ascendc_src_file = "lp_norm_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator LpNormV2. kernel name is " + kernel_name)
    op_type = "LpNormV2"
...
def op_select_format(x_in__, y_out_, p=2, axes=[], keepdim=False, epsilon=1e-12, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, p, axes, keepdim, epsilon)
    result = check_op_cap("op_select_format", "LpNormV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, p=2, axes=[], keepdim=False, epsilon=1e-12, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, p, axes, keepdim, epsilon)
    result = check_op_cap("get_op_specific_info", "LpNormV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
