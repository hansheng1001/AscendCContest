...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnLessEqualGetWorkspaceSize(inputTensor_[0], inputTensor_[1], outputTensor_[0],
	...
	INFO_LOG("Execute aclnnLessEqualGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnLessEqual(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
