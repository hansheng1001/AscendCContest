...
OperatorDesc CreateOpDesc() {
  ...
  OperatorDesc opDesc;
  opDesc.AddInputTensorDesc(dataTypeInput, shape.size(), shape.data(), format);
  opDesc.AddInputTensorDesc(dataTypeInput, shape.size(), shape.data(), format);
  opDesc.AddOutputTensorDesc(dataTypeOut, shape.size(), shape.data(), format);
  return opDesc;
}
...
bool ProcessOutputData(OpRunner &runner) {
  WriteFile("../output/output_z.bin", runner.GetOutputBuffer<void>(0),
