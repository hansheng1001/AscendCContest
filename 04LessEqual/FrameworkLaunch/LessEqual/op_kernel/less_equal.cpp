...
public:
  ...
  __aicore__ inline void Init(GM_ADDR x1, GM_ADDR x2, GM_ADDR y,
    ...
    if (tilingKey == 1) {
      ...
      dyGm.SetGlobalBuffer((__gm__ DTYPE_X1 *)x1 +
                               this->blockLength * GetBlockIdx(),
                           this->blockLength);

      xGm.SetGlobalBuffer((__gm__ DTYPE_X2 *)x2 +
                              this->blockLength * GetBlockIdx(),
                          this->blockLength);

      zGm.SetGlobalBuffer((__gm__ DTYPE_Y *)y +
                              this->blockLength * GetBlockIdx(),
                          this->blockLength);
    }
...
private:
  ...
  __aicore__ inline void Compute(int32_t progress) {
    ...
    LocalTensor<DTYPE_X> x1Local = inLocal;
    LocalTensor<DTYPE_X> x2Local = inLocal[this->tileLength];
