...
def _build_args(x1_in__, x2_in__, y_out_):
    __inputs__ = []
    for arg in [x1_in__, x2_in__]:
        if arg != None:
...
@tbe_register.register_operator("LessEqual", trans_bool_to_s8=False)
...
def less_equal(x1_in__, x2_in__, y_out_, kernel_name="less_equal", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_)
    ...
    origin_func_name = "less_equal"
    ascendc_src_dir_ex = "less_equal"
    ascendc_src_dir = "less_equal"
    ascendc_src_file = "less_equal.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator LessEqual. kernel name is " + kernel_name)
    op_type = "LessEqual"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__], origin_outputs = [y_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x1_in__, x2_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_)
    result = check_op_cap("op_select_format", "LessEqual", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "LessEqual", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
