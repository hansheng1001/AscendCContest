...
def gen_golden_data_simple():
    input_var = np.random.uniform(1, 10, [64]).astype(np.float16)
    input_indices  = np.random.uniform(0, 63, [4]).astype(np.int32)
    input_updates = np.random.uniform(-10, 0, [4]).astype(np.float16)
    use_locking = False
