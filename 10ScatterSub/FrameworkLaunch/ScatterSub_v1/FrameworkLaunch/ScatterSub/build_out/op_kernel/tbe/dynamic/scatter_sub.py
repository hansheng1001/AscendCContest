...
def _build_args(x_in__, index_in__, update_in__, use_locking):
    __inputs__ = []
...
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def scatter_sub(x_in__, index_in__, update_in__, use_locking=False, kernel_name="scatter_sub", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, index_in__, update_in__, use_locking)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, index_in__, update_in__], origin_outputs = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, index_in__, update_in__, use_locking=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, index_in__, update_in__, use_locking)
    result = check_op_cap("op_select_format", "ScatterSub", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, index_in__, update_in__, use_locking=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, index_in__, update_in__, use_locking)
