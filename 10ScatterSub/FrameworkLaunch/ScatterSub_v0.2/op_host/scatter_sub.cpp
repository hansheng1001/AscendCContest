...
static ge::graphStatus TilingFunc(gert::TilingContext *context) {
  ...
  for (int i = 0; i < x1_shape->GetStorageShape().GetDimNum(); i++) {
    indexLength *= index_shape->GetStorageShape().GetDim(i);
  }

  tiling.set_indexLength(indexLength);

  auto Tnsor_index = context->GetInputTensor(1);
  auto index_addr = Tnsor_index->GetData<int>();
  for (int i = 0; i < indexLength; i++) {
    std::cout << "index[" << i + 1 << "]: " << index_addr[i] << std::endl;
  }

  std::cout << std::endl;

  // 获取updates的长度, 并设置updateType
  const gert::StorageShape *updates_shape = context->GetInputShape(2);
  int32_t updatesLength = 1;
  for (int i = 0; i < x1_shape->GetStorageShape().GetDimNum(); i++) {
    updatesLength *= index_shape->GetStorageShape().GetDim(i);
  }

  // tiling.set_indexLength(updatesLength);
  tiling.set_updateType(0); // 默认为类型0
  if (1 == updatesLength) {
    tiling.set_updateType(1);
  }
  ...
  case 0:
    sizeofdatatype = 4;
    // tiling.set_dataType_(0);
    break;

    // DT_FLOAT16
  case 1:
    sizeofdatatype = 2;
    // tiling.set_dataType_(1);
    break;

    // DT_INT8 = 2, // int8 type
  case 2:
    // tiling.set_dataType_(2);
    sizeofdatatype = 1;
    break;

    // DT_INT32 = 3, // int32 type
  case 3:
    // tiling.set_dataType_(3);
    sizeofdatatype = 4;
    break;
  ...
  std::cout << "indexLength:" << indexLength
            << ", updatesLength:" << updatesLength << ", dataType:" << aivNum
            << std::endl;
  ...
  if ((totalLengthAligned / ALIGN_NUM) % block_dim == 0) { //核间可均分
    ...
    if ((formerLength / ALIGN_NUM) % ub_block_num == 0 ||
      ...
      if (formerLength < ub_block_num * ALIGN_NUM) {
        // formertileLength = ((formerLength / ALIGN_NUM) + 1) / 2 * 2 *
        // ALIGN_NUM;
        formertileLength =
            (formerLength + (ALIGN_NUM - 1)) / ALIGN_NUM * ALIGN_NUM;
    ...
    if ((tailLength / ALIGN_NUM) % ub_block_num == 0 ||
      ...
      if (tailLength < (ub_block_num * ALIGN_NUM)) {
        ...
        tailtileLength = (tailLength + (ALIGN_NUM - 1)) / ALIGN_NUM * ALIGN_NUM;
