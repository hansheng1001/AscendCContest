...
def _build_args(x_in__, index_in__, update_in__, x_out_, use_locking):
    __inputs__ = []
    for arg in [x_in__, index_in__, update_in__]:
        if arg != None:
    ...
    if use_locking != None:
        attr = {}
        attr["name"] = "use_locking"
        attr["dtype"] = "bool"
        attr["value"] = use_locking
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("ScatterSub", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def scatter_sub(x_in__, index_in__, update_in__, x_out_, use_locking=False, kernel_name="scatter_sub", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, index_in__, update_in__, x_out_, use_locking)
    ...
    origin_func_name = "scatter_sub"
    ascendc_src_dir_ex = "scatter_sub"
    ascendc_src_dir = "scatter_sub"
    ascendc_src_file = "scatter_sub.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator ScatterSub. kernel name is " + kernel_name)
    op_type = "ScatterSub"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, index_in__, update_in__], origin_outputs = [x_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, index_in__, update_in__, x_out_, use_locking=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, index_in__, update_in__, x_out_, use_locking)
    result = check_op_cap("op_select_format", "ScatterSub", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, index_in__, update_in__, x_out_, use_locking=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, index_in__, update_in__, x_out_, use_locking)
    result = check_op_cap("get_op_specific_info", "ScatterSub", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
