...
extern "C" __global__ __aicore__ void mse_loss(GM_ADDR x, GM_ADDR y,
  ...
  half *f16_x = reinterpret_cast<half *>(v_x);
  half *f16_y = reinterpret_cast<half *>(v_y);
  half *f16_out = reinterpret_cast<half *>(v_out);
  ...
  for (int i = 0; i < USE_CORE_NUM; i++) {
    ...
    for (int j = 0; j < sumCount; j++) {
      memcpy(&t, out + offset + j * sizeof(half), sizeof(half));
      sum += t;
    }
  ...
  memcpy(out, &sum, sizeof(half));

  // WriteFile("./output/output_out.bin", out, outputByteSize);
  WriteFile("./output/output_out.bin", out, 2);
